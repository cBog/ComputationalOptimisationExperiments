...
public class BaseFunctions
{
	...
	*/
	public static class Sphere extends Problem
	{
		...
		*/
		public double f(double[] x)
		{
			...
			{
				// Loop through each decision variable, square it and add this to the output.
				// (﻿Math.pow can be much less efficient implementation for non-fractional powers)
				for (int i = 0; i < n; i++)
					y += x[i] * x[i];
			}
			return y;
	...
	public static class Schwefel extends Problem
	{
		...
		*/
		public Schwefel(int dimension) { super(dimension, new double[] {-500.0, 500.0}); }
		/**
		...
		*/
		public double f(double[] x)
		{
			...
			{
				// create the sum part first
				for (int i=0; i<n; i++)
					sum += x[i] * Math.sin(Math.sqrt(Math.abs(x[i])));

				//then minus it from the constant
				y = (418.9829 * n) - sum;
			}
			return y;
		}
	...
	{
		/**
		 * Costructor for the  Rastrigin function with suggested bounds.
		 */
